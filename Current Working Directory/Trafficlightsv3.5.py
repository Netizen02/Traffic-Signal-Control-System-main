...
class Car:
    ...
    def check_turn(self):
        ...
        # sample xpos <= WIDTH/2-0.75*ROAD_WIDTH and xpos+self.x_vel >= (WIDTH/2-0.75*ROAD_WIDTH)) and (ypos == ROAD_WIDTH/4):
        # calc actual x,y --=- self.get_actual_pos(xpos,ypos)
        # self.x = actual x, y...
        # self.x_vel = abs(self.y_vel)
        # self.y_vel = 0
        ...
        x, y = self.get_center()
        if y == ROAD_WIDTH/4 and x <= WIDTH/2 - 0.75*ROAD_WIDTH and x+self.x_vel >= WIDTH/2 - 0.75*ROAD_WIDTH:
            self.x, self.y = self.get_actual_pos(x, y)
            self.x_vel = abs(self.y_vel)
            self.y_vel = 0

        elif x == WIDTH/2 - 0.75*ROAD_WIDTH and y <= 1.5*ROAD_WIDTH and y+self.y_vel >= 1.5*ROAD_WIDTH:
            self.x, self.y = self.get_actual_pos(x, y)
            self.y_vel = abs(self.x_vel)
            self.x_vel = 0

        elif y == 1.5*ROAD_WIDTH and x >= WIDTH/2 - 0.25*ROAD_WIDTH and x+self.x_vel <= WIDTH/2 - 0.25*ROAD_WIDTH:
            self.x, self.y = self.get_actual_pos(x, y)
            self.x_vel = -abs(self.y_vel)
            self.y_vel = 0

        elif x == WIDTH/2 - 0.25*ROAD_WIDTH and y <= 2.5*ROAD_WIDTH and y+self.y_vel >= 2.5*ROAD_WIDTH:
            self.x, self.y = self.get_actual_pos(x, y)
            self.y_vel = abs(self.x_vel)
            self.x_vel = 0

        elif y == 2.5*ROAD_WIDTH and x >= WIDTH/2 + 0.25*ROAD_WIDTH and x+self.x_vel <= WIDTH/2 + 0.25*ROAD_WIDTH:
            self.x, self.y = self.get_actual_pos(x, y)
            self.x_vel = -abs(self.y_vel)
            self.y_vel = 0

        elif x == WIDTH/2 + 0.25*ROAD_WIDTH and y <= 3.5*ROAD_WIDTH and y+self.y_vel >= 3.5*ROAD_WIDTH:
            self.x, self.y = self.get_actual_pos(x, y)
            self.y_vel = abs(self.x_vel)
            self.x_vel = 0

        elif y == 3.5*ROAD_WIDTH and x <= WIDTH/2 + 0.75*ROAD_WIDTH and x+self.x_vel >= WIDTH/2 + 0.75*ROAD_WIDTH:
            self.x, self.y = self.get_actual_pos(x, y)
            self.x_vel = abs(self.y_vel)
            self.y_vel = 0

        elif x == WIDTH/2 + 0.75*ROAD_WIDTH and y <= 4.5*ROAD_WIDTH and y+self.y_vel >= 4.5*ROAD_WIDTH:
            self.x, self.y = self.get_actual_pos(x, y)
            self.y_vel = -abs(self.x_vel)
            self.x_vel = 0

    def get_actual_pos(self):
        return(self.x + ROAD_WIDTH/8, self.Y +ROAD_WIDTH/8)
